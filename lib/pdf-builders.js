...
export function buildMobileHtml(catalog, origin) {
  ...
  for (const spec of productPages) {
    ...
    pages.push(`<article${spec.first ? ` id="mobile-category-${e(spec.category.id)}"` : ""} class="mobile-page mobile-product" style="--category-color:${color(spec.category)}" data-mode="${e(product.imageMode || "contain")}">${header("mobile-header", spec.category.short || spec.category.name, product.name, spec.number)}<div class="mobile-product-image"><img src="${e(localProductImage(product))}" alt="${e(product.name)}" style="object-position:${e(product.imagePosition || "center center")}"></div><div class="mobile-product-copy"><div class="mobile-tags">${tags}</div><div class="mobile-description"><h2>${product.detailsLabel === "Descripción comercial" ? "Descripción" : "Ingredientes"}</h2><p>${e(product.ingredients)}</p>${extra ? `<strong>${e(extra)}</strong>` : ""}</div><div class="mobile-meta">${weight}<div><small>Formato</small><strong>${e(product.format)}</strong></div><div><small>Valor neto</small><strong>${e(product.price)}</strong></div></div><a class="mobile-whatsapp" href="${e(productWhatsapp)}">Consultar por WhatsApp <span>↗</span></a></div></article>`);
  }
