...
function documentHtml(title, cssFiles, body, origin, bodyClass = "") {
  const styles = cssFiles.map(file => fs.readFileSync(path.join(ROOT, file), "utf8")).join("\n");
  const base = `${String(origin || "http://127.0.0.1:8080").replace(/\/$/, "")}/`;
  return `<!doctype html><html lang="es"><head><meta charset="utf-8"><base href="${e(base)}"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${e(title)}</title><style>${styles}</style></head><body class="${e(bodyClass)}">${body}</body></html>`;
}
...
function productCard(catalog, product, compact = false) {
  ...
  return `<article class="spread-product ${compact ? "compact" : ""}" data-mode="${e(product.imageMode || "contain")}">
    ...
    <div class="spread-meta">${weight}<div><small>Formato</small><strong>${e(product.format)}</strong></div><div><small>Valor neto</small><strong>${e(product.price)}</strong></div></div></div></article>`;
}
...
export function buildA4Html(catalog, origin) {
  ...
  pages.push(`<article class="pdf-page closing-page" id="contacto"><img class="closing-photo" src="${vertical}" alt=""><div class="closing-panel"><img src="assets/brand/logo-principal.png" alt="Vestalia"><p>${e(meta.contactEyebrow || "Pedidos para cafeterías")}</p><h1>${editorialTitle(meta.contactTitle || "¿Qué ponemos en tu vitrina?")}</h1><span>${e(meta.contactText || "Consulta sabores, disponibilidad y coordinación de despacho.")}</span><div class="closing-contact"><a href="${e(catalog.contact.whatsappUrl)}">WhatsApp · ${e(catalog.contact.whatsapp || catalog.contact.phone)}</a><a href="${e(catalog.contact.instagramUrl)}">${e(catalog.contact.instagram)}</a><a href="${e(catalog.contact.phoneUrl)}">${e(catalog.contact.phone)}</a><a href="${e(catalog.contact.emailUrl)}">${e(catalog.contact.email)}</a></div><blockquote>${e(meta.mantra)}</blockquote></div></article>`);
  return { html: documentHtml(`${meta.brand} — ${meta.title}`, ["print.css", "weasyprint.css"], `<main id="print-catalog">${pages.join("")}</main>`, origin, "print-document"), pageCount: pages.length };
}
...
export function buildMobileHtml(catalog, origin) {
  ...
  const digits = String(catalog.contact.whatsapp || catalog.contact.phone || "").replace(/\D/g, "");
  const whatsapp = catalog.contact.whatsappUrl || `https://wa.me/${digits}`;
  for (const spec of productPages) {
    ...
    pages.push(`<article${spec.first ? ` id="mobile-category-${e(spec.category.id)}"` : ""} class="mobile-page mobile-product" style="--category-color:${color(spec.category)}" data-mode="${e(product.imageMode || "contain")}">${header("mobile-header", spec.category.short || spec.category.name, product.name, spec.number)}<div class="mobile-product-image"><img src="${e(localProductImage(product))}" alt="${e(product.name)}" style="object-position:${e(product.imagePosition || "center center")}"></div><div class="mobile-product-copy"><div class="mobile-tags">${tags}</div><div class="mobile-description"><h2>${product.detailsLabel === "Descripción comercial" ? "Descripción" : "Ingredientes"}</h2><p>${e(product.ingredients)}</p>${extra ? `<strong>${e(extra)}</strong>` : ""}</div><div class="mobile-meta">${weight}<div><small>Formato</small><strong>${e(product.format)}</strong></div><div><small>Valor neto</small><strong>${e(product.price)}</strong></div></div><a class="mobile-whatsapp" href="${e(`${whatsapp}${separator}text=${message}`)}">Consultar por WhatsApp <span>↗</span></a></div></article>`);
  }
  ...
  pages.push(`<article class="mobile-page mobile-contact"><img class="contact-pattern" src="${pattern}" alt=""><div class="contact-copy"><img src="assets/brand/logo-principal.png" alt="Vestalia"><p>${e(meta.contactEyebrow || "Pedidos para cafeterías")}</p><h1>${editorialTitle(meta.contactTitle || "¿Qué ponemos en tu vitrina?")}</h1><span>${e(meta.contactText || "Consulta sabores y disponibilidad.")}</span><div><a href="${e(whatsapp)}">WhatsApp · ${e(catalog.contact.whatsapp || catalog.contact.phone)}</a><a href="${e(catalog.contact.instagramUrl)}">${e(catalog.contact.instagram)}</a><a href="${e(catalog.contact.emailUrl)}">${e(catalog.contact.email)}</a></div><blockquote>${e(meta.mantra)}</blockquote><b>${String(contactPage).padStart(2, "0")}</b></div></article>`);
  return { html: documentHtml(`${meta.brand} — ${meta.title} móvil`, ["mobile-print.css"], `<main>${pages.join("")}</main>`, origin, "mobile-document"), pageCount: pages.length };
}
