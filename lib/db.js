...
export async function ensureDatabase() {
  ...
  if (!initialization) initialization = (async () => {
    ...
    await sql`CREATE TABLE IF NOT EXISTS catalog_state (
      id smallint PRIMARY KEY CHECK (id = 1), catalog jsonb NOT NULL, revision bigint NOT NULL DEFAULT 1,
      pdf_revision bigint NOT NULL DEFAULT 1, pdf_a4_url text NOT NULL, pdf_mobile_url text NOT NULL,
    ...
    await sql`INSERT INTO catalog_state (id, catalog, revision, pdf_revision, pdf_a4_url, pdf_mobile_url, pdf_updated_at)
      VALUES (1, CAST(${bundled} AS jsonb), 1, 1, '/Vestalia_Catalogo_Cafeterias.pdf', '/Vestalia_Catalogo_Movil.pdf', NOW())
...
export async function getCatalogState() {
  if (!hasDatabase()) {
    return {
      catalog: loadBundledCatalog(), revision: 1, pdfRevision: 1,
