...
export const CATALOG_CHANNELS = ["cafeterias", "personas"];

export function normalizeCatalogChannel(value) {
  return String(value || "").toLowerCase() === "personas" ? "personas" : "cafeterias";
}
...
export function loadBundledCatalog(channel = "cafeterias") {
  const normalized = normalizeCatalogChannel(channel);
  const filename = path.join(process.cwd(), "data", normalized === "personas" ? "catalogo-personas.json" : "catalogo.json");
